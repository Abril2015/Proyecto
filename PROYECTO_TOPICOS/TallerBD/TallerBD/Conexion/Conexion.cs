...
namespace TallerBD.Conexion
{
    ...
    {
        SqlConnection conexion = new SqlConnection(@"Data Source=DESKTOP-3G4P0P9\SQLEXPRESS;Initial Catalog=SUPER;Integrated Security=True");
        ...
        public String insertarArticulo(String clave_art, String costo, String descripcion, String marca, String modelo, String stock, String depto)
        {
            try
            {
                ...
                SqlDataReader registros = comando.ExecuteReader();
                String respuesta = "Error al insertar producto";
                while (registros.Read())
                {
                    //Regresar mensaje del procedimiento almacenado
                    respuesta = registros.GetString(0);
        ...
        public String eliminarArticulo(String clave)
        {
            try
            {
                ...
                SqlCommand comando = new SqlCommand(query, conexion);
                comando.ExecuteNonQuery();
                return "El producto se eliminò correctamente";
            }
            catch (SqlException ex)
            {
                if (ex.Number == 547)
                {
                    return "No sé puede eliminar este producto debido a que está relacionado";
                }
                return ex.Message;
